...
class TranscriptionServer:
    ...
    def initialize_client(
        ...
        logging.info(f"is translation enabled: {enable_translation}")
        if enable_translation:
            ...
            translation_client = ServeClientTranslation(
                ...
                model_name=translation_model_path,
            )
    ...
    def get_audio_from_websocket(self, websocket):
        ...
        if frame_data == b"USER_MSG_ENDED":
            response = {
                "uid": "",
                "status": "COMPLETE",
                "message": "USER_MSG_ENDED",
            }
            logging.info(
                f"User msg ended. Ack to client to start TTS pipeline: {response}"
            )
            websocket.send(json.dumps(response))
            return True
    ...
    def handle_new_connection(
        ...
        try:
            ...
            logging.info("options json: ", options)
