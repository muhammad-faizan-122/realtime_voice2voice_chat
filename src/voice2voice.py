...
class SharedState:
    IS_RECV_LAST_SEG = False
    IS_USER_MSG_COMPLETED = False
    USER_MSG = []
...
class Client:
    ...
    def __init__(
        ...
        send_last_n_segments=10,  # unclear
        no_speech_thresh=0.45,
        clip_audio=False,
        same_output_threshold=10,  # unclear
    ...
    def process_segments(self, segments, translated=False):
        ...
        for i, seg in enumerate(segments):
            ...
            # if (first segment) or (last appended text segment not equal to current text segment)
            if not text or text[-1] != seg["text"]:
                text.append(seg["text"].strip())

                # if (last index) AND (current segment completed key value is FALSE)
                if (i == len(segments) - 1) and (not seg.get("completed", False)):
                    self.last_segment = seg

                # if (backend is faster whisper) AND (current segment completed key value is TRUE)
                elif self.server_backend == "faster_whisper" and seg.get(
                    "completed", False
                ):
                    if translated:
                        # log.debug("processing for translated...")
                        if not self.translated_transcript or float(
                            seg["start"]
                        ) >= float(self.translated_transcript[-1]["end"]):
                            self.translated_transcript.append(seg)

                    else:
                        # log.debug(f"processing for transcript...{self.transcript}")
                        # if (first transcripted segment) or (last appended text segment not equal to current text segment)
                        if not self.transcript or float(seg["start"]) >= float(
                            self.transcript[-1]["end"]
                        ):
                            if SharedState.IS_USER_MSG_COMPLETED:
                                SharedState.IS_RECV_LAST_SEG = True
                                log.info(
                                    f"RECEIVED LAST SEGMENT...{SharedState.IS_RECV_LAST_SEG}"
                                )

                            log.debug(f"completed segment: {seg}")
                            SharedState.USER_MSG.append(seg["text"])
        ...
        if self.log_transcription:
            ...
            if (
                self.last_segment is not None
                and self.last_segment["text"] not in original_text
            ):
    ...
    def on_message(self, ws, message):
        ...
        if (
            ...
            and message.get("message", "") == "USER_MSG_ENDED"
        ):
            log.info(f"FULLY RECEIVED USER MSG...{SharedState.IS_USER_MSG_COMPLETED}")
            SharedState.IS_USER_MSG_COMPLETED = True
        ...
        if "message" in message.keys() and message["message"] == "DISCONNECT":
            log.info("[INFO]: Server disconnected due to overtime.")
            self.recording = False
        ...
        if "message" in message.keys() and message["message"] == "SERVER_READY":
            ...
            log.info(f"[INFO]: Server Running with backend {self.server_backend}")
            return
        ...
        if "segments" in message.keys():
            # log.debug(f"segment: {message['segments']}")
    ...
    def send_packet_to_server(self, message, opcode):
        """
    ...
    def write_srt_file(self, output_path="output.srt"):
        ...
        if self.server_backend == "faster_whisper":
            if not self.transcript and self.last_segment is not None:
                # log.debug(f"last segment: {self.last_segment}")
                # log.debug(f"transcript all segment: {self.transcript}")
        ...
        if self.enable_translation:
            utils.create_srt_file(
                self.translated_transcript, self.translation_srt_file_path
            )
...
class Voice2VoiceClient:
    ...
    def multicast_packet(
        ...
        for client in self.clients:
            if unconditional or client.recording:
                client.send_packet_to_server(packet, opcode)

    def is_user_pause(self, audio_array, threshold=0.09):
        """
        Checks if the audio array contains speech based on simple energy threshold.
        """
    ...
    def record(self):
        ...
        CHECK_EVERY_SEC = 3
        SILENCE_WINDOW_SEC = 5
        ENERGY_THRESHOLD = 0.09

        BYTES_PER_SECOND = self.rate * 2  # mono * 2 bytes (16-bit)
        MAX_WINDOW_BYTES_LEN = CHECK_EVERY_SEC * BYTES_PER_SECOND
        TRIM_BYTES_LEN = SILENCE_WINDOW_SEC * BYTES_PER_SECOND
        ...
        try:
            for _ in range(0, int(self.rate / self.chunk * self.record_seconds)):
                ...
                if len(window_bytes) > MAX_WINDOW_BYTES_LEN:

                    last_window = window_bytes[-TRIM_BYTES_LEN:]
                    audio_array = self.bytes_to_float_array(last_window)
                    ...
                    if self.is_user_pause(
                        ...
                        while not SharedState.IS_RECV_LAST_SEG:
                            time.sleep(0.1)

                        log.info("INVOKING TTS PIPELINE...")
...
class Voice2Voice(Voice2VoiceClient):
    ...
    Example:
        To create a VoiceAgent and start transcription on microphone audio:
        ```python
        transcription_client = VoiceAgent(host="localhost", port=9090)
